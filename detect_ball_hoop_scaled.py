#!/usr/bin/env python3
"""
Ball detection using hoop-scaled color blob analysis.
...
- Defines a small ROI around hoop (configurable in feet).
...
OUT_DIR = "sample_frames_hoop_scaled"
OUT_CSV = "sample_frames_hoop_scaled_detections.csv"
...
# ROI in feet around hoop (adjust as needed)
HALF_WIDTH_FT = 1.5   # left/right of hoop
HALF_HEIGHT_FT = 2.5  # up/down of hoop
...
def detect_ball_in_frame(bgr, hoop_center, hoop_radius_px):
    ...
    for cnt in contours:
        ...
        diam_est = 2 * radius
        # Optional size filter: expect diameter within 0.5x to 2x expected
        if not (0.5 * expected_diam_px <= diam_est <= 2.0 * expected_diam_px):
            continue
