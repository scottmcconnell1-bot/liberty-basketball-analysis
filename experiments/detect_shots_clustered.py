...
SIGNAL_CLUSTER_SEC = 1.0  # cluster signals within this window (seconds)
FPS = 3.75
INTERP_MAX_GAP = 15
...
def main():
    ...
    output_csv = f'{base}/final_events_clustered.csv'
    ...
    print(f'Total referee signals: {len(signal_df)}')
    ...
    # Cluster signals in time
    if len(signal_rows) == 0:
        signal_clusters = []
    else:
        # sort by time
        signal_rows = signal_rows.sort_values('timestamp_ms').reset_index(drop=True)
        clusters = []
        current_cluster = [signal_rows.iloc[0]]
        for i in range(1, len(signal_rows)):
            prev_time = signal_rows.iloc[i-1]['timestamp_ms']
            curr_time = signal_rows.iloc[i]['timestamp_ms']
            if (curr_time - prev_time) <= SIGNAL_CLUSTER_SEC * 1000:
                current_cluster.append(signal_rows.iloc[i])
            else:
                clusters.append(current_cluster)
                current_cluster = [signal_rows.iloc[i]]
        clusters.append(current_cluster)
        signal_clusters = []
        for cluster in clusters:
            times = [row['timestamp_ms'] for row in cluster]
            signals = [row['signal'] for row in cluster]
            # determine if any both-hands (signal==2) in cluster
            made = any(s == 2 for s in signals)
            attempt_type = '3PT'  # all clusters are attempt signals (since we filtered 1,2)
            # time of cluster: average
            avg_time_ms = np.mean(times)
            signal_clusters.append({
                'time_ms': avg_time_ms,
                'frame': int(avg_time_ms * FPS / 1000.0),
                'type_init': '3PT',
                'made_init': made,
                'source': 'signal_cluster'
            })
    
    print(f'After clustering, {len(signal_clusters)} signal clusters')
    for sc in signal_clusters[:5]:
        print(f"  time {sc['time_ms']/1000:.2f}s, type {sc['type_init']}, made {sc['made_init']}")
    ...
    for aframe, atime in zip(attempt_frames_2pt, attempt_times_2pt):
        attempt_2pt.append({
            'frame': int(aframe),
            'time_ms': atime * 1000.0,  # store in ms for consistency
            'type_init': '2PT',
    ...
    # Combine attempts
    all_attempts = signal_clusters + attempt_2pt
    # Sort by time_ms
    all_attempts.sort(key=lambda x: x['time_ms'])
    # Now resolve duplicates: if two attempts are within 0.5 seconds, keep the one from signal if any, else keep the first.
    DEDUP_SEC = 0.5
    final_attempts = []
    i = 0
    while i < len(all_attempts):
        current = all_attempts[i]
        # look ahead for duplicates within DEDUP_SEC
        j = i + 1
        while j < len(all_attempts) and (all_attempts[j]['time_ms'] - current['time_ms']) / 1000.0 <= DEDUP_SEC:
            j += 1
        # now i..j-1 are duplicates within DEDUP_SEC
        # prefer signal source if any
        signal_candidates = [k for k in range(i, j) if all_attempts[k]['source'] == 'signal_cluster']
    ...
    print(f'After deduplication ({DEDUP_SEC}s), {len(final_attempts)} unique shot attempts')
    
    # Now associate shooter and classify (may override based on distance) and compute makes
    events = []
    for att in final_attempts:
        aframe = att['frame']
        atime_ms = att['time_ms']
        atime = atime_ms / 1000.0
        ...
        # Override: if shot_init was 3PT from signal, we keep it as 3PT regardless of distance? We'll keep classification for now.
