...
def main(video_path, output_csv, stride=1):
    ...
    with PoseLandmarker.create_from_options(options) as landmarker:
        ...
        while True:
            ...
            left_wrist = {'x': None, 'y': None, 'visibility': 0.0}
            right_wrist = {'x': None, 'y': None, 'visibility': 0.0}
            left_shoulder = {'x': None, 'y': None, 'visibility': 0.0}
            right_shoulder = {'x': None, 'y': None, 'visibility': 0.0}
            signal = 0  # none
            if pose_landmarker_result.pose_landmarks:
                # Take the first pose (assuming one person)
                lm = pose_landmarker_result.pose_landmarks[0]
                # Indices: left ankle=27, right ankle=28, left wrist=15, right wrist=16, left shoulder=11, right shoulder=12
                left_ankle_lm = lm[27]
                right_ankle_lm = lm[28]
                left_wrist_lm = lm[15]
                right_wrist_lm = lm[16]
                left_shoulder_lm = lm[11]
                right_shoulder_lm = lm[12]
                # Ankle visibility
                ...
                if left_wrist_lm.visibility > 0.5:
                    left_wrist = {'x': left_wrist_lm.x, 'y': left_wrist_lm.y, 'visibility': left_wrist_lm.visibility}
                if right_wrist_lm.visibility > 0.5:
                    right_wrist = {'x': right_wrist_lm.x, 'y': right_wrist_lm.y, 'visibility': right_wrist_lm.visibility}
                # Shoulder visibility
                if left_shoulder_lm.visibility > 0.5:
                    left_shoulder = {'x': left_shoulder_lm.x, 'y': left_shoulder_lm.y, 'visibility': left_shoulder_lm.visibility}
                if right_shoulder_lm.visibility > 0.5:
                    right_shoulder = {'x': right_shoulder_lm.x, 'y': right_shoulder_lm.y, 'visibility': right_shoulder_lm.visibility}
...
if __name__ == '__main__':
    ...
    output_csv = sys.argv[2] if len(sys.argv) > 2 else '/home/monk-admin/PROJECTS/liberty-basketball-analysis/pose_keypoints_full.csv'
