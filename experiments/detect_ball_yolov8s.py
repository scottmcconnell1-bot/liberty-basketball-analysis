...
def main(video_path, output_csv, conf_thresh=0.01, stride=4):
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Usage: python detect_ball_yolov8s.py <video_path> <output_csv> [conf_thresh] [stride]')
        sys.exit(1)
    video_path = sys.argv[1]
    output_csv = sys.argv[2]
    conf_thresh = float(sys.argv[3]) if len(sys.argv) > 3 else 0.01
