...
ball_csv = 'ball_q1_conf00005_stride8.csv'
...
# Smooth ball trajectory (we have sparse detections, so we need to interpolate)
frames = ball_df['frame'].values
raw_x = ball_df['x'].values
raw_y = ball_df['y'].values
# We'll create a full frame range from min to max and interpolate
min_frame = int(frames.min())
max_frame = int(frames.max())
all_frames = np.arange(min_frame, max_frame+1)
# Interpolate x and y
from scipy import interpolate
def interpolate_series(frames, values, all_frames):
    # frames and values are arrays; we want to interpolate to all_frames
    # Use linear interpolation, fill extrapolated with NaN
    interp = interpolate.interp1d(frames, values, kind='linear', bounds_error=False, fill_value=np.nan)
    return interp(all_frames)
smooth_x_raw = interpolate_series(frames, raw_x, all_frames)
smooth_y_raw = interpolate_series(frames, raw_y, all_frames)
# Now smooth with moving window
def smooth_series(arr, window=SMOOTH_WINDOW):
    return pd.Series(arr).rolling(window, center=True, min_periods=1).mean().values
smooth_x = smooth_series(smooth_x_raw, window=SMOOTH_WINDOW)
smooth_y = smooth_series(smooth_y_raw, window=SMOOTH_WINDOW)
...
def main():
    ...
    ball_csv = f'{base}/ball_q1_conf00005_stride8.csv'
    signal_csv = f'{base}/signal_full.csv'
    db_path = f'{base}/film_analysis.db'
    output_csv = f'{base}/final_events_lowconf_signal.csv'
    
    # Reload data (we already have, but for clarity)
    ball_df = pd.read_csv(ball_csv)
    ...
    # Recompute smooth and vy (just in case)
    frames = ball_df['frame'].values
    raw_x = ball_df['x'].values
    raw_y = ball_df['y'].values
    min_frame = int(frames.min())
    ...
    all_frames = np.arange(min_frame, max_frame+1)
    def interpolate_series(frames, values, all_frames):
        interp = np.interp(all_frames, frames, values, left=np.nan, right=np.nan)
        return interp
    smooth_x_raw = interpolate_series(frames, raw_x, all_frames)
    smooth_y_raw = interpolate_series(frames, raw_y, all_frames)
    smooth_x = smooth_series(smooth_x_raw, window=SMOOTH_WINDOW)
    ...
    for _, row in signal_df.iterrows():
        ...
        end_frame = min(len(smooth_y)-1, int(signal_frame) + int(SIGNAL_WINDOW_SEC * FPS))
    ...
    for ev in events:
        ...
        end = min(len(smooth_y)-1, f + int(0.5 * FPS))
    ...
    for peak_idx in peak_candidates:
        ...
        for i in range(search_start, search_end):
            if smooth_y[i] > hoop_threshold_px and vy[i] > 0:  # crossing downward
                attempt_frames_2pt.append(all_frames[i])  # note: all_frames index corresponds to smooth_y index
                attempt_times_2pt.append(all_frames[i] / FPS)
    ...
    for aframe, atime in attempt_2pt_filtered:
        # Associate shooter
        shooter_ft = associate_shooter(int(aframe), person_df)
        hoop_center_ft = hoop_center_px * scale_ft_per_px
