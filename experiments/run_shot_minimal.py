...
while True:
    ...
    if fn % 500 == 0:
        ...
        print(f'Progress: {fn}/{total} frames, {nb} ball det, {nc} color-ok, {elapsed:.1f}s')
...
for fn in deduped:
    d = dist_to_basket[fn]
    bc = ball_conf[fn]
    print(f'  F{fn:4d}: dist={d if not np.isnan(d) else -1:6.1f}px conf={bc:.3f}')
...
with open(os.path.join(OUT, 'shot_v7.pkl'), 'wb') as f:
    pickle.dump(results, f)

csv_path = os.path.join(OUT, 'shot_candidates_v7.csv')
...
print(f'\nDone. Candidates written to {csv_path}')