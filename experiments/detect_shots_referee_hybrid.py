...
BALL_NEAR_HOOP_THRESH_PX = 100  # increased threshold for ball near hoop to count as signal-associated attempt
FPS = 3.75
INTERP_MAX_GAP = 15
...
def main():
    ...
    output_csv = f'{base}/final_events_referee_hybrid.csv'
    ...
    for _, row in signal_df.iterrows():
        ...
        for f in range(start_frame, end_frame+1):
            ...
            if dist_to_hoop < BALL_NEAR_HOOP_THRESH_PX:
                near_hoop = True
                break
        # We'll consider it a 3PT attempt if ball near hoop OR we always trust the signal? 
        # For now, we require ball near hoop to avoid false positives, but we can adjust.
        if near_hoop:
            ...
            # If no ball near hoop, we still might want to consider it as a 3PT attempt? 
            # We'll skip for now to reduce false positives.
            pass
