#!/usr/bin/env python3
"""Test B: Same clip at 720p. Existing v14 detector. Stride=10. Just measure."""
...
CONF = 0.0002
IOU = 0.3
# Court region for 720p (original values from v14)
COURT_X_MIN, COURT_X_MAX = 100, 1180
COURT_Y_MIN, COURT_Y_MAX = 100, 650
...
print(f"\n=== TEST B RESULTS (720p, stride={STRIDE}) ===")
...
out = {
    'source': 'NFHS 720p downscaled', 'stride': STRIDE, 'total_frames': total,
    'frames_processed': processed, 'ball_count': ball_count,
    'avg_infer_s': avg_infer, 'total_time_s': elapsed,
    'detections': results
}
with open('pipeline_output/testB_720p_str10.pkl', 'wb') as f:
    pickle.dump(out, f)

df = pd.DataFrame(results, columns=['frame', 'cx', 'cy', 'conf'])
df.to_csv('pipeline_output/testB_720p_str10.csv', index=False)
if len(results) > 0:
