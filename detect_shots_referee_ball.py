...
def main():
    ...
    output_csv = f'{base}/final_events_referee_ball.csv'
    ...
    # Process referee signals for 3PT (only rows where signal is 1 or 2)
    attempt_frames_3pt = []
    attempt_times_3pt = []
    ...
    print(f'Detected {len(attempt_frames_3pt)} 3PT shot attempts from referee signals (with ball near hoop)')
    
    # Combine attempts, avoiding duplicates: if a referee signal frame is within +/- 1 second of a 2PT attempt, we treat it as the 3PT attempt (priority to signal)
    # We'll first collect all attempts with tentative types, then resolve conflicts.
    all_attempts = []  # each dict with frame, time, type_init, made_init (from signal), etc.
    ...
    for aframe, atime, made_init in zip(attempt_frames_3pt, attempt_times_3pt, made_3pt_from_signal):
        all_attempts.append({
            'frame': aframe,
            'time': atime,
            'type_init': '3PT',
    ...
    for att in final_attempts:
        ...
        # Override: if shot_init was 3PT from signal, we might keep it as 3PT regardless of distance? Let's keep classification for now.
