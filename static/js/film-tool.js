...
function setActiveRosterSeasonId(seasonId) {
    activeRosterSeasonId = seasonId ? String(seasonId) : '';
    saveJson(ROSTER_SEASON_STORAGE_KEY, activeRosterSeasonId);
    if (rosterSeasonSelect && activeRosterSeasonId) rosterSeasonSelect.value = activeRosterSeasonId;
}
...
function fillRosterSeasonSelect(selectEl, selectedId = '') {
    ...
    if (!rosterSeasonOptions.length) {
        const opt = document.createElement('option');
        opt.value = '';
        opt.textContent = 'No seasons found — add one on Schedule';
        selectEl.appendChild(opt);
        return;
    }
    rosterSeasonOptions.forEach(season => {
        const opt = document.createElement('option');
...
function queueRosterImport(file) {
    if (!file) return;
...
async function confirmRosterImport() {
    const file = pendingRosterImportFile;
    const seasonId = rosterImportSeasonSelect?.value;
    if (!file) return;
    ...
    try {
        const resp = await fetch('/api/film-rosters/import', { method: 'POST', body: formData });
        const data = await resp.json();
...
function seasonLabel(seasonId) {
    const season = rosterSeasonOptions.find(item => String(item.id) === String(seasonId));
...
async function clearCurrentRoster() {
    ...
    const key = getRosterKey();
    const count = (rosters[key] || []).length;
    if (!count) return;
    ...
    try {
        ...
        await persistRosters();
        showRoster();
        setStatus(`Cleared roster for ${seasonLabel(seasonId)}.`);
    } catch (err) {
        alert(err.message || 'Could not clear roster.');
    }
}
...
async function loadReviewPlayers() {
    try {
        ...
        if (!response.ok) throw new Error('players failed');
        aiReviewPlayersCache = await response.json();
    } catch (_err) {
        aiReviewPlayersCache = [];
    }
...
function renderCorrectPlayerManageList() {
    ...
    list.innerHTML = aiReviewPlayersCache.map(player => `
        <div class="ai-correct-player-row" data-player-id="${player.id}">
            <span>${escapeHtml(player.name || '')}${player.jersey_number != null ? ` #${escapeHtml(String(player.jersey_number))}` : ''}</span>
            <button class="btn btn-ghost btn-sm" type="button" data-delete-review-player="${player.id}">Delete</button>
        </div>
    `).join('');
    list.querySelectorAll('[data-delete-review-player]').forEach(btn => {
        btn.addEventListener('click', async (event) => {
            event.preventDefault();
            const playerId = btn.dataset.deleteReviewPlayer;
            if (!confirm('Delete this player from tagging list?')) return;
            try {
                const response = await fetch(`/api/players/${playerId}`, { method: 'DELETE' });
                if (!response.ok) throw new Error('delete failed');
                await loadReviewPlayers();
                const current = document.getElementById('aiCorrectPlayer')?.value || '';
                populateCorrectPlayerSelect(current);
                renderCorrectPlayerManageList();
                setStatus('Player deleted.');
            } catch (_err) {
                setStatus('Could not delete player.');
            }
        });
    });
}
...
async function submitAiCorrection(formEvent) {
...
function initAiUpload() {
    ...
    function doUpload(fileOrBlob, url, filename) {
        ...
        // Use chunked upload for files > 40MB to stay under Cloudflare's ~100MB proxy limit
        // Each chunk is 40MB raw → ~42MB with multipart overhead, well under 100MB
    ...
    function doChunkedUpload(file, url, filename, opponent, chunkSize) {
        ...
        function uploadNextChunk(chunkIndex) {
            ...
            xhr.upload.addEventListener('progress', (e) => {
                ...
                const chunkBytesLoaded = e.loaded;
                const overallPercent = Math.round(((bytesUploaded + chunkBytesLoaded) / file.size) * 100);
                const loadedMB = ((bytesUploaded + chunkBytesLoaded) / 1024 / 1024).toFixed(1);
                const totalMB = (file.size / 1024 / 1024).toFixed(1);
                if (uploadProgressBar) uploadProgressBar.style.width = `${overallPercent}%`;
                if (uploadProgressText) uploadProgressText.textContent = `Uploading chunk ${chunkIndex + 1}/${totalChunks} — ${overallPercent}% (${loadedMB} / ${totalMB} MB)`;
            });

            xhr.addEventListener('load', () => {
                if (xhr.status < 200 || xhr.status >= 300) {
                    failed = true;
                    const errMsg = xhr.responseText ? xhr.responseText.substring(0, 200) : '';
                    if (uploadProgressText) uploadProgressText.textContent = `Upload failed on chunk ${chunkIndex + 1} (HTTP ${xhr.status})${errMsg ? ': ' + errMsg : ''}. Please try again.`;
                    return;
                }
                let payload;
                try { payload = JSON.parse(xhr.responseText); } catch (e) {
                    failed = true;
                    if (uploadProgressText) uploadProgressText.textContent = `Upload failed on chunk ${chunkIndex + 1}: invalid server response. Please try again.`;
                    return;
                }
            ...
            xhr.addEventListener('error', () => { failed = true; if (uploadProgressText) uploadProgressText.textContent = `Network error on chunk ${chunkIndex + 1}. Check your connection and try again.`; });
            xhr.addEventListener('timeout', () => { failed = true; if (uploadProgressText) uploadProgressText.textContent = `Chunk ${chunkIndex + 1} timed out. Try again or use a smaller file.`; });
            xhr.addEventListener('abort', () => { failed = true; if (uploadProgressText) uploadProgressText.textContent = `Upload cancelled on chunk ${chunkIndex + 1}.`; });
            xhr.send(formData);
        }
...
function attachEventHandlers() {
    ...
    document.getElementById('aiCorrectForm')?.addEventListener('submit', submitAiCorrection);
    document.getElementById('aiCorrectAddPlayerBtn')?.addEventListener('click', async () => {
        const name = (document.getElementById('aiCorrectNewPlayerName')?.value || '').trim();
        const jerseyRaw = document.getElementById('aiCorrectNewPlayerNum')?.value;
        if (!name) { setStatus('Player name required.'); return; }
        const body = { name };
        if (jerseyRaw !== '' && jerseyRaw != null) body.jersey_number = Number(jerseyRaw);
        try {
            const response = await fetch('/api/players', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!response.ok) throw new Error('add player failed');
            document.getElementById('aiCorrectNewPlayerName').value = '';
            document.getElementById('aiCorrectNewPlayerNum').value = '';
            await loadReviewPlayers();
            populateCorrectPlayerSelect(name);
            renderCorrectPlayerManageList();
...
function init() {
    ...
    rosterSeasonSelect = document.getElementById('rosterSeasonSelect');
    rosterImportDialog = document.getElementById('rosterImportDialog');
